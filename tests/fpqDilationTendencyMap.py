...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 30 11:57:13 2020

fpqDilationTendency.py
...
if not bFilename:
    sys.exit('Usage: python3 fpqDilationTendency.py -I<inputfilename>')
...
for t in range(0, nTraces): 
    ...
    for s in range(1, nSegsThis):
        ...
        Td = (fSigma1 - sn) / (fSigma1 - fSigma2)
        iTd = int(round(Td*100)) 
        
        segcolours[segcount] = colours[iTd] 
        segcount += 1
...
plt.grid(bGrid)
plt.colorbar(sc, label='Dilation tendency', 
                 orientation='horizontal', 
                 fraction=0.04)
p.set_clim(0, 1)
plt.title('Dilation tendency map, n=%i' % nSegs)
plt.savefig("fpqDilationTendencyMap.png", dpi=600)
